...
class ImageScrollView: UIScrollView, UIScrollViewDelegate {
    ...
    @objc func setImage() {
        DispatchQueue.global().async {
            guard let urlString = self.imageUrl, let url = URL(string: urlString) else {
                self.setImageToImageView(image: #imageLiteral(resourceName: "placeholder"))
                return
            }
            let data = try? Data(contentsOf:url)
            if let data = data {
                self.setImageToImageView(image:  UIImage(data: data) ?? #imageLiteral(resourceName: "placeholder"))
            } else {
                self.setImageToImageView(image: #imageLiteral(resourceName: "placeholder"))
            }
        }
    ...
    private func setImageToImageView(image: UIImage) {
        DispatchQueue.main.async {
            ...
            self._imageView?.frame = CGRect(x: 0, y: 0, width: (self._imageView!.image?.size.width)!, height: (self._imageView!.image?.size.height)!)
            self.configureForImageSize((self._imageView!.image?.size)!)
        }
    }
    ...
    func setMaxMinZoomScalesForCurrentBounds() {
        ...
        if _imageSize != nil {
            ...
            let maxScale: CGFloat = 5.0 / UIScreen.main.scale
