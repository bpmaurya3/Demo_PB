...
public struct SlideMenuOptions {
    public static var leftViewWidth: CGFloat = 261.5
    ...
    public static var rightViewWidth: CGFloat = 261.5
    ...
    // Note Flickering Animation needs to uncomment
    // mainContainerView.transform - in all place commented
}
...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    @discardableResult
    func userlogOutStatus(status: @escaping UpdateLogoutStatusClosur) -> Self {
        self.logoutStatus = status
        return self
    }
    
    typealias UpdateLogedInStatusClosur = ((Bool) -> Void)
    var logInStatus: UpdateLogedInStatusClosur = { _ in }
    @discardableResult
    func userlogedInStatus(status: @escaping UpdateLogedInStatusClosur) -> Self {
    ...
    @discardableResult
    func userlogedInSkipStatus(status: @escaping UpdateLogInSkipActionClosur) -> Self {
    ...
    open func topViewController() -> UIViewController? {
        let mainNavVC = self.mainViewController
        let topVC = mainNavVC?.topViewController
        return topVC
    }
    ...
    open override func openLeft() {
        guard leftViewController != nil else { // If leftViewController is nil, then return
            return
        }
    ...
    open override func openRight() {
        guard rightViewController != nil else { // If rightViewController is nil, then return
            return
        }
        ...
        if let rigntVC = rightViewController as? RightMenuViewController {
            rigntVC.updateLogOutStatus(status: { [weak self] (isLogedOut) in
                self?.logoutStatus(isLogedOut)
            }) .updateLogInStatus(status: { [weak self] (isLogedIn) in
                self?.logInStatus(isLogedIn)
            }) .updateLogInSkipActionStatus(status: { [weak self] (skipAction) in
                self?.logInSkipStatus(skipAction)
            })
...
//                SlideMenuOptions.contentViewDrag == true ? (strongSelf.mainContainerView.transform = CGAffineTransform(translationX: -SlideMenuOptions.rightViewWidth, y: 0)) : (strongSelf.mainContainerView.transform = CGAffineTransform(scaleX: SlideMenuOptions.contentViewScale, y: SlideMenuOptions.contentViewScale))
    ...
    open func changeMainViewController(_ mainViewController: UIViewController, close: Bool) {
        if let main = self.mainViewController {
            main.popToRootViewController(animated: false)
            main.pushViewController(mainViewController, animated: false)
        }
    ...
    open func changeToRootMainViewController(_ mainViewController: UIViewController, close: Bool) {
        if let main = self.mainViewController {
            main.setViewControllers([mainViewController], animated: true)
        }
    }

    open func popToRootViewController(close: Bool) {
        if let main = self.mainViewController {
            main.popToRootViewController(animated: false)
        }
